...
function resolveCSS(htmlStr) {
  ...
  // console.log(newCSS);
  // 3.4 调用 fs.writeFile() 方法，将提取的样式，写入到 clock 目录 index.css 文件中
  fs.writeFile(
...
function resolveJS(htmlStr) {
  ...
  // console.log(newJS);
  // 4.4 调用 fs.writeFile() 方法，将提取的脚本，写入到 clock 目录 index.js 文件中
...
function resolveHTML(htmlStr) {
  // 5.2 使用字符串的 replace 方法，将内嵌的 style 和 script 标签，替换为外联的 link 和 script 标签
  ...
  fs.writeFile(
    ...
    newHTML,
    function (err) {
      if (err) return console.log("写入 HTML 结构失败！" + err.message);
